...
_VALID_OUTPUT = {
    "unit": "function",
    "name": "_canon",
    "file": "python/amesh_vault_client/amesh_vault_client/_envelope.py",
...
_VALID_METHOD_OUTPUT = {
    "unit": "method",
    "name": "sign",
    "file": "python/amesh_vault_client/amesh_vault_client/_client.py",
...
class TestCodeSignatureValid:
    ...
    def test_well_formed_amesh_style(self):
        # Real signature from amesh holdout
        output = {**_VALID_OUTPUT, "signature": "_canon(obj: Any)"}
        assert code_signature_valid(output) is True
...
# ── Self-consistency test on real amesh data ──────────────────────────────

_AMESH_HOLDOUT = (
    Path(__file__).parent.parent / "data" / "repos" / "amesh" / "holdout.jsonl"
)


class TestAmeshSelfConsistency:
    """Verify that scoring each amesh holdout record against itself yields perfect scores."""

    @staticmethod
    def _load_slice(n: int = 50):
        if not _AMESH_HOLDOUT.exists():
            pytest.skip("amesh holdout.jsonl not present")
        records = []
        with open(_AMESH_HOLDOUT) as f:
    ...
    def test_self_consistency_format_and_field_accuracy(self):
        records = self._load_slice(50)
        for rec in records:
            output = rec.get("output")
            result = score_code_record(output, output or {})
            assert result["format_score"] == 1.0, (
                f"format fail for record: {rec.get('output')}"
            )
            assert result["field_accuracy"] == 1.0, (
                f"field_accuracy fail for record: {rec.get('output')}"
            )
    ...
    def test_self_consistency_with_signature_check(self):
        ...
        for rec in records:
            output = rec.get("output")
            result = score_code_record(output, output or {}, check_signature=True)
            assert result["format_score"] == 1.0
            assert result["field_accuracy"] == 1.0
            # All amesh signatures are well-formed (generated by scanner)
            assert result["signature_valid"] is not False, (
                f"unexpected malformed signature: {output.get('signature') if output else None}"
            )
